...
function parseMusicPath(path: string): ParsedMusic {
  // Remove BGM/ prefix and .wav extension
  const cleanPath = path.replace(/^BGM\//, '').replace(/\.wav$/, '');
...
function MusicListItem({
  ...
  return (
    <button
      onClick={onClick}
      title={music.path} // Show full path on hover
    ...
    >
      <div className='flex items-center gap-1.5'>
        ...
        <span className='font-medium truncate'>{music.displayName}</span>
      </div>
    </button>
  );
}
...
export const MusicView: React.FC<MusicViewProps> = () => {
  const { data: music = [], isLoading: isLoadingList } = useMusic();
  const [selectedMusic, setSelectedMusic] = useState<string | null>(null);

  const groupedMusic = useMemo(() => {
    const parsed = music.map(parseMusicPath);
  ...
  const renderContent = () => {
    ...
    const audioUrl = `https://raw.githubusercontent.com/HaiKonofanDesu/konofan-audio/refs/heads/main/${selectedMusic}`;
    const parsedTrack = parseMusicPath(selectedMusic);
        ...
        <div className='max-w-lg w-full p-8'>
          <div className='bg-white/10 backdrop-blur-md rounded-lg p-6'>
            <div className='flex justify-center mb-6'>
              <Music className='w-16 h-16 text-purple-900/40' />
            </div>
            <div className='text-center mb-6'>
              <div className='text-sm text-purple-900/60 mb-1'>{parsedTrack.category}</div>
              <div className='text-lg font-medium text-purple-900'>{parsedTrack.displayName}</div>
            </div>
        ...
        <div className='flex-1 overflow-y-auto custom-scrollbar'>
          {isLoadingList ? (
            ...
            <div className='divide-y divide-white/5'>
              {groupedMusic.map(({ category, items }) => (
                <div key={category}>
                  ...
                  <div className='p-2 space-y-0.5'>
                    {items.map(music => (
                      <MusicListItem
                        key={music.path}
                        music={music}
                        isSelected={selectedMusic === music.path}
                        onClick={() => setSelectedMusic(music.path)}
                      />
                    ))}
                  </div>
                </div>
